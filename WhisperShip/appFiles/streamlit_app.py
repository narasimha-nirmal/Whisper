...
from utils.data import load_raw, filter_ca, filter_national
from page.sic_lookup     import sic_lookup_page
from page.single_sic     import single_sic_comparison_page
from page.multi_sic      import multi_sic_clustering_page

st.set_page_config(layout="wide", page_title="Sector Dashboard")

df_raw = load_raw()
df_ca  = filter_ca(df_raw)
df_nat = filter_national(df_raw)

PAGES = {
    "🔎 SIC Lookup (All)":        (sic_lookup_page,        df_raw),
    "📊 Single SIC (CA)":         (single_sic_comparison_page, df_ca),
    "🌳 Multi-SIC Cluster (CA)":  (multi_sic_clustering_page,  df_ca),
    "📊 Single SIC (US)":         (single_sic_comparison_page, df_nat),
    "🌳 Multi-SIC Cluster (US)":  (multi_sic_clustering_page,  df_nat),
}

st.sidebar.title("📄 Pages")
choice = st.sidebar.radio("Go to", list(PAGES.keys()))
page_func, data = PAGES[choice]
page_func(data)
...
# #   a) California
# # California filter in one string
# df_ca = df_raw.query(
#     "FY >= 2018 "
#     "and STATE == 'CA' "
...
# # National (no STATE filter) likewise:
# df_nat = df_raw.query(
#     "FY >= 2018 "
#     "and 100_000 < ANNUAL_ELECTRICITY_COST < 3_000_000 "
#     "and TOTAL_RECC_SAVINGS >= 100_000 "
...
# # ---- Page registry ----
# PAGES = {
#     "SIC Lookup":        lambda: sic_lookup_page(df_raw),
#     "CA: Single SIC Compare":lambda: single_sic_comparison_page(df_ca),
#     "CA: Multi-SIC Cluster": lambda: multi_sic_clustering_page(df_ca),
#     "US: Single SIC Compare":lambda: single_sic_comparison_page(df_nat),
#     "US: Multi-SIC Cluster": lambda: multi_sic_clustering_page(df_nat),
# }
